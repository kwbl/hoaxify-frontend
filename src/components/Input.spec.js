...
describe("Layout", () => {
  ...
  it("does not display label when no label provided in props", () => {
    const { container } = render(<Input />)
    const label = container.querySelector("label")
    expect(label).not.toBeInTheDocument()
  })

  it("has text type for input when type is not propvided as prop", () => {
    const { container } = render(<Input />)
    const input = container.querySelector("input")
    expect(input.type).toBe("text")
  })

  it("has password type for input when password type is propvided as prop", () => {
    const { container } = render(<Input type="password" />)
    const input = container.querySelector("input")
    expect(input.type).toBe("password")
  })

  it("displays placeholder when it is provided as a prop", () => {
    const { container } = render(<Input placeholder="Test placeholder" />)
    const input = container.querySelector("input")
    expect(input.placeholder).toBe("Test placeholder")
  })

  it("has value for input when it is provided as a prop", () => {
    const { container } = render(<Input value="Test value" />)
    const input = container.querySelector("input")
    expect(input.value).toBe("Test value")
  })

  it("has onChange callback when it is provided as a prop", () => {
